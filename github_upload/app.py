...
def _get_user(username: str, branch_key: str):
    conn = _auth_db(branch_key)
    cur = conn.cursor(dictionary=True)
    cur.execute(
        """
...
def api_login():
    ...
    branch_key = get_default_branch_key()
    ...
    user = _get_user(username, branch_key)
...
def api_usuarios():
    ...
    for user in users:
        user["branch_key"] = session.get("branch_key")
        user["branch_name"] = branch_name(session.get("branch_key"))
        user["is_admin"] = bool(int(user.get("is_admin") or 0))
