...
import subprocess
from confirmation_requests import register_confirmation_module
...
DEFAULT_BRANCH_KEY = os.getenv("DEFAULT_BRANCH_KEY", "central").strip() or "central"
...
def _auth_db(branch_key: str | None = None):
    # La autenticacion es comun: el usuario define a que sucursal entra.
...
def get_branch_config(branch_key: str | None = None) -> dict:
    branch = BRANCHES.get(branch_key or session.get("branch_key") or DEFAULT_BRANCH_KEY)
    if branch:
        return branch
    return BRANCHES["central"]


def get_default_branch_key() -> str:
    return DEFAULT_BRANCH_KEY if DEFAULT_BRANCH_KEY in BRANCHES else "central"
...
def _get_user(username: str):
    ...
    cur.execute(
        """
        SELECT id, username, password_hash, display_name, email, telefono, activo, is_admin, sucursal_key
        FROM usuarios
        WHERE username=%s
        LIMIT 1
        """,
...
def _current_user_row():
    ...
    cur.execute(
        """
        SELECT id, username, display_name, email, telefono, activo, is_admin, sucursal_key
        FROM usuarios
        WHERE id=%s
        LIMIT 1
        """,
...
def _session_payload() -> dict:
    branch_key = session.get("branch_key") or get_default_branch_key()
...
def login_required(view):
    ...
    def wrapped(*args, **kwargs):
        ...
        if not branch_key or branch_key not in BRANCHES:
...
register_confirmation_module(
    app,
    branches=BRANCHES,
    shared_db_name=SHARED_DB_NAME,
    build_db_config=build_db_config,
    get_db=get_db,
    shared_table=shared_table,
    login_required=login_required,
    insert_hist=_insert_hist,
    to_upper=to_upper,
)
...
def api_login():
    ...
    branch_key = str((user or {}).get("sucursal_key") or get_default_branch_key()).strip()
    if branch_key not in BRANCHES:
        return jsonify({"ok": False, "error": "El usuario no tiene una sucursal valida asignada"}), 403
...
def api_switch_branch():
    return jsonify({"ok": False, "error": "El cambio de sucursal está deshabilitado. Cerrá sesión e ingresá en la sucursal correspondiente."}), 403
...
def api_me_update():
    ...
    conn = _auth_db(session.get("branch_key"))
    cur = conn.cursor(dictionary=True)
...
def api_usuarios():
    ...
    cur.execute(
        ...
        SELECT id, username, display_name, email, telefono, activo, is_admin, sucursal_key
        FROM usuarios
        ORDER BY sucursal_key, display_name, username
        """
    )
    users = cur.fetchall()

    for user in users:
        user["branch_key"] = user.get("sucursal_key") or get_default_branch_key()
        user["branch_name"] = branch_name(user["branch_key"])
        user["is_admin"] = bool(int(user.get("is_admin") or 0))
...
def api_usuarios_crear():
    ...
    branch_key = str(data.get("branch_key") or data.get("sucursal_key") or "").strip()
    ...
    if branch_key not in BRANCHES:
        return jsonify({"ok": False, "error": "Elegí una sucursal válida"}), 400
...
def api_usuarios_actualizar(user_id: int):
    ...
    params = [username, display_name, email or None, telefono or None, activo, is_admin, branch_key]
    sql = """
        UPDATE usuarios
        SET username=%s,
            display_name=%s,
