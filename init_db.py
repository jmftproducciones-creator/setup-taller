...
from confirmation_requests import get_confirmation_table_sql
...
RESET_DEFAULT_USER_PASSWORDS = os.getenv("RESET_DEFAULT_USER_PASSWORDS", "0").strip().lower() in ("1", "true", "yes", "si", "s")
...
AUTH_TABLES = [
    ...
    CREATE TABLE IF NOT EXISTS usuarios (
        ...
        sucursal_key VARCHAR(50) NOT NULL DEFAULT 'central',
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
def setup_shared_tables() -> None:
    ...
    ensure_column(conn, "clientes", "direccion", "VARCHAR(255) NULL AFTER nombre")
    ensure_column(conn, "clientes", "localidad", "VARCHAR(120) NULL AFTER direccion")
    ensure_column(conn, "clientes", "provincia", "VARCHAR(120) NULL AFTER localidad")
    ensure_column(conn, "clientes", "cp", "VARCHAR(30) NULL AFTER provincia")
    ensure_column(conn, "clientes", "telefono", "VARCHAR(60) NULL AFTER cp")
    ensure_column(conn, "clientes", "celular", "VARCHAR(60) NULL AFTER telefono")
    ensure_column(conn, "clientes", "email", "VARCHAR(255) NULL AFTER celular")
    ensure_column(conn, "clientes", "cuit", "VARCHAR(60) NULL AFTER email")
    ensure_column(conn, "clientes", "contacto", "VARCHAR(120) NULL AFTER cuit")
    ensure_column(conn, "clientes", "observaciones", "TEXT NULL AFTER contacto")
    ensure_column(conn, "clientes", "giro_empresa", "VARCHAR(160) NULL AFTER observaciones")
    ensure_column(conn, "clientes", "cliente_garantia", "TINYINT(1) NOT NULL DEFAULT 0 AFTER giro_empresa")
    ensure_column(conn, "clientes", "cliente_con_contrato", "TINYINT(1) NOT NULL DEFAULT 0 AFTER cliente_garantia")

    ensure_column(conn, "equipos", "descripcion", "VARCHAR(255) NULL")
    ensure_column(conn, "equipos", "serie", "VARCHAR(160) NULL AFTER descripcion")
    ensure_column(conn, "equipos", "serie_norm", "VARCHAR(160) NULL AFTER serie")
    ensure_column(conn, "equipos", "tipo", "VARCHAR(120) NULL AFTER serie_norm")
    ensure_column(conn, "equipos", "marca", "VARCHAR(120) NULL AFTER tipo")
    ensure_column(conn, "equipos", "modelo", "VARCHAR(120) NULL AFTER marca")
    conn.close()


def setup_auth_database(branch_key: str, branch: dict) -> None:
    auth_database = branch["database"]
    ...
    ensure_column(conn, "usuarios", "sucursal_key", "VARCHAR(50) NOT NULL DEFAULT 'central' AFTER is_admin")

    cur = conn.cursor()

    for visible_key, visible_branch in BRANCHES.items():
        cur.execute(
            """
            ...
            ON DUPLICATE KEY UPDATE
                ...
                activa=1
            """,
            (visible_key, visible_branch["name"], visible_branch["theme"]),
        )

    password_update_sql = ",\n                password_hash=VALUES(password_hash)" if RESET_DEFAULT_USER_PASSWORDS else ""

    for user in DEFAULT_USERS:
        if branch_key != "central":
            continue
        cur.execute(
            f"""
            INSERT INTO usuarios (username, display_name, email, telefono, password_hash, activo, is_admin, sucursal_key)
...
def migrate_branch_users_to_shared_auth() -> None:
    shared_database = BRANCHES["central"]["database"]
    conn = server_connection(shared_database)
    cur = conn.cursor(dictionary=True)

    for branch_key, branch in BRANCHES.items():
        source_database = branch["database"]
        if source_database == shared_database:
            continue
        cur.execute(
            """
            SELECT 1
        ...
        cur.execute(
            f"""
            INSERT IGNORE INTO `{shared_database}`.`usuarios`
                (username, display_name, email, telefono, password_hash, activo, is_admin, sucursal_key)
            SELECT username, display_name, email, telefono, password_hash, activo, is_admin, %s
            FROM `{source_database}`.`usuarios`
            """,
            (branch_key,),
        )
...
def main() -> None:
    ...
    for branch_key, branch in BRANCHES.items():
        setup_auth_database(branch_key, branch)
    migrate_branch_users_to_shared_auth()
    print(f"Bases listas: usuarios por sucursal, sucursales={', '.join(branch['database'] for branch in BRANCHES.values())}")
