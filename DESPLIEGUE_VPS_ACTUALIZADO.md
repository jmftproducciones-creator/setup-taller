...
RESET_DEFAULT_USER_PASSWORDS=0
...
Si el login devuelve `401` con el usuario inicial despues de una actualizacion, probablemente ya existia un usuario con otra clave. Para regenerar las claves iniciales una sola vez:

```bash
RESET_DEFAULT_USER_PASSWORDS=1 python init_db.py
```

Luego dejá `RESET_DEFAULT_USER_PASSWORDS=0` en `.env` para no pisar cambios de contraseña hechos desde el panel.
