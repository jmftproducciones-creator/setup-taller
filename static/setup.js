...
function resetLoginAttempt(message = "") {
  hideLoading(true);
  setAuthLocked(true);
...
async function cargarOrdenEnFormulario(ordenId) {
  const id = String(ordenId || "").trim();
  if (!id) return;

  let orden = listaOrdenes.find(o => String(o.id) === id);
  if (!orden) {
    const resp = await fetchJSON(`/api/ordenes/${id}`);
    if (!resp || !resp.ok) {
      showToast("No se pudo cargar la orden", "error");
      return;
    }
    orden = await resp.json();
  }
  ...
  const buscar = document.getElementById("buscar_nro");
  if (buscar) buscar.value = orden.id || id;
  irATabFormulario();
  actualizarAccionesOrdenUI();
}
...
function limpiarFormularioUsuarios() {
  ...
  setValue("usuario_branch_key", sessionInfo.branch?.key || "central");
...
function escribirFormularioUsuario(u) {
  ...
  setValue("usuario_password", "");
  setValue("usuario_branch_key", u.branch_key || u.sucursal_key || sessionInfo.branch?.key || "central");
...
function leerFormularioUsuario() {
  return {
    ...
    branch_key: document.getElementById("usuario_branch_key")?.value || "",
  ...
  listaUsuarios
    ...
    .forEach(u => {
      ...
      tr.innerHTML = `
        ...
        <td>${escapeHTML(u.branch_name || sessionInfo.branch?.name || "")}</td>
...
function hideLoading(force = false) {
  ...
  __loadingCount = force ? 0 : Math.max(0, __loadingCount - 1);
  if (__loadingCount === 0) {
...
async function guardarUsuario() {
  ...
  loginForm?.addEventListener("submit", async (e) => {
    ...
    if (r.ok === false) {
      resetLoginAttempt(r.error || "Usuario o contraseña incorrectos");
  ...
  document.getElementById("branchSwitcher")?.addEventListener("change", async (e) => {
    const branchSwitcher = e.currentTarget;
